...
void ApplicationInstance::createSurface()
{
    ...
    auto surface = surfaceManager->createSurface(surfaceName,
           ...
           this,
           QUrl(m_application->screenshot()),
           m_application->qmlFilePath());
    ...
    connect(surface, &MirSurfaceInterface::focusedChanged, this, [&](bool /*value*/) {
        #if APPLICATION_DEBUG
        qDebug().nospace() << "ApplicationInstance[" << m_application->appId() << "].focusedChanged(" << focused() << ")";
        #endif
...
void ApplicationInstance::setFullscreen(bool value)
{
    ...
    if (m_surfaceList->rowCount() > 0) {
        static_cast<MirSurface*>(m_surfaceList->get(0))->setState(Mir::FullscreenState);
    }
}
...
void ApplicationInstance::setFocused(bool value)
{
    ...
    if (value) {
        if (m_surfaceList->count() > 0) {
            static_cast<MirSurface*>(m_surfaceList->get(0))->requestFocus();
        }
    } else {
        for (int i = 0; i < m_surfaceList->count(); ++i) {
            MirSurface *surface = static_cast<MirSurface*>(m_surfaceList->get(i));
...
void ApplicationInstance::createPromptSurface()
{
    if (state() == ApplicationInstance::Stopped) { return; }
