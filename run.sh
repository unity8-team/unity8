...
usage() {
    ...
    echo " -o, --performanceoverlay Display the performance overlay." >&2
    echo >&2
    exit 1
}

ARGS=`getopt -n$0 -u -a --longoptions="fake,pinlock,keylock,gdb,help,lightdm:,nomousetouch" -o "fpkl:ghmo" -- "$@"`
[ $? -ne 0 ] && usage
eval set -- "$ARGS"
...
do
    case "$1" in
       ...
       -o|--performanceoverlay) PERFORMANCE_OVERLAY=true;;
       --)         shift;break;;
    esac
    shift
done
...
if $PERFORMANCE_OVERLAY; then
  QML_PHONE_SHELL_ARGS="$QML_PHONE_SHELL_ARGS -performanceoverlay"
fi
